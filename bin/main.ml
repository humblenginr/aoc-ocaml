  ...
  (* part-2 *)
 let is_overlapping assign_pair = match assign_pair with 
      | first :: second :: _ -> begin let parts1 = String.split_on_char '-' first in let parts2 = String.split_on_char '-' second in
                                ...
                                | s1, s2, _, e2 when s1 > s2 -> if s1 > e2 then false else true
                                | s1, s2, e1, _ when s1 < s2 -> if s2 > e1 then false else true
                                | _ -> assert false end
      | _ :: [] -> assert false
      | [] -> assert false
 ...
 let is_contained assign_pair = match assign_pair with 
  ...
  let solve input_list = let rec helper il total = match il with 
      | [] -> total
      | assign_pair :: rest -> if is_overlapping assign_pair then helper rest (total + 1) else helper rest total 
